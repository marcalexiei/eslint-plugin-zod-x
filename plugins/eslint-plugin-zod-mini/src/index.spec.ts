...
describe('recommended config', () => {
  it('has correct shape', () => {
    ...
    expect(recommendedConfig.name).toBe('eslint-plugin-zod-mini/recommended');
    expect(recommendedConfig.plugins).toHaveProperty('zod-mini');
  ...
  it('only references registered, non-deprecated rules', () => {
    for (const { configRuleName, rule } of getRecommendedRuleEntries(plugin)) {
      expect(configRuleName).toMatch(/^zod-mini\//);
